...
$entrycount=$dbcon->getexec("SELECT COUNT(1) FROM tbltestimonial ".$strwhere);
...
$testimoniallist=$dbcon->exec("SELECT tbltestimonial.*, tblcourse.title FROM tbltestimonial LEFT JOIN tblcourse ON tblcourse.id = tbltestimonial.course_id ".$strwhere." ORDER BY tbltestimonial.date_created DESC LIMIT ".$position.", ".ITEM_PER_PAGE);
                      ...
                      <div class="ibox-content">
                          <div class="table-responsive">
                              <table class="table table-striped table-bordered table-hover datatables-content" >
                                  ...
                                  <tbody>
  <?php
    for($i=0;$i<$testimoniallist;$i++){
      $row=$dbcon->data_seek($i);
  ?>
                                  <tr id="item-<?php echo $row[id];?>">
                                      <td><img src="<?php echo extractfile($row[image], 'preview', '60x60%23'); ?>" class="img-thumbnail" /></td>
                                      <td><?php echo $row[person_name]; ?></td>
                                      <td><?php echo truncate($row[content], 100); ?></td>
                                      <td><?php echo $row[title]; ?></td>
                                      <td><?php echo date('j M Y h:i:s A', $row[date_created]); ?></td>
                                      <td class="text-center">
                                        <div class="btn-group action-tooltip">
                                          <a href="testimonial.php?testimonialid=<?php echo $row[id]; ?>" class="btn-white btn btn-sm" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-pencil"></i></a>
                                        </div>
                                      </td>
                                  </tr>
